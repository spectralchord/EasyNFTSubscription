...
const DetailsPage = () => {
        ...
        try {
            ...
            for (let i = max; i >= Math.max(min, 0); i--) {
                ...
                arr.push({
                    ...
                    uris: batchInfo[0]?.map((item: string) => item.replace("ipfs://", 'https://w3s.link/ipfs/')),
                    batchNr: i + 1
                })
            }
    ...
    const subscribeContract = async () => {
        if ((!isConnected || isConnecting)) {
            ...
            toast({
                title: 'Error!',
                description: `Switch network to Areon Testnet!`,
                variant: 'destructive'
            })
        }
    }
    ...
    const getNfts = async (min: number, max: number, initialData: any,) => {
        ...
        try {
            ...
            for (let i = max; i >= Math.max(min, 0); i--) {
                ...
                const res = await fetch( tokenUri.replace("ipfs://", 'https://w3s.link/ipfs/'))
                const data = await res.json()
                arr.push({
                    id: parseInt(tokenId),
                    uri: data?.image.replace('ipfs://', 'https://w3s.link/ipfs/'),
