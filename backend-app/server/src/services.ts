...
import {SubscriptionContractAbi} from "./web3/abi/SubscriptionContractAbi";
import axios from "axios";
import {HttpsProxyAgent} from 'https-proxy-agent';
import {Buffer} from 'buffer'
import {SUBSCRIPTION_TIME_INTERVAL, SUBSCRIPTIONS_CONTRACTS, SUBSCRIPTIONS_RELAYERS_KEYS} from "./web3/config";
...
const generateAndSendNft = async ({contractAddress, signer}: { contractAddress: string, signer: Signer }) => {
    ...
    resImage.data.map((item, index) => {
        let obj = {
            ...
            image: "https://w3s.link/ipfs/" + item.path.slice(34),
        }
    ...
    const uriArray = resMetadata.data.map(item => "https://w3s.link/ipfs/" + item.path.slice(34))
